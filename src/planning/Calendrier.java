...
public class Calendrier {
	...
	private void initialize() {
		...
		about.setURLupdate("Recherche de mise � jour", "www.sdtp.com/versions/version.php?program=jcalendrier&version="+Config.version+"."+Config.build, "Derni�re recherche de mise � jour", Config.lastupdchk);
	...
	private String setLabelDay(DateTime dt) {
	    ...
	    if (ss.length() > 0) {
	    	s += ss+" � "+Quarter.YearDays.get(day-1).seasondate.toString(fmt)+ "<br>";
	    }
	    // Scolar holidays
	    ...
	    s += "Lever et coucher du soleil � "+Config.town+": "+sunrise.toString(fmt)+" - "+sunset.toString(fmt);	
	...
	public void setSeasons(int year) {
		...
		if (!Beans.isDesignTime()) {
			...
			String s = "<html>Printemps: " +Astro.DateTimeToString(prin, "HH:mm")+"<br>"+
						"Et�: "+Astro.DateTimeToString(ete, "HH:mm")+"</html>";
